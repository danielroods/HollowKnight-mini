...
public class GuideMenuScreen extends BaseMenuScreen {
    ...
    private static final float CONTENT_TOP_Y = 820f;
    ...
    private static final String[][] CONTROLS = {
        {"Left/Right", "Move"},
        {"Space", "Jump  -  Double Jump in air  -  Wall Jump off a wall"},
        {"X", "Nail Attack"},
        {"Up + X", "Nail Attack (Upward)"},
        {"Down + X", "Nail Attack (Downward)"},
        {"C", "Dash"},
        {"A (Hold)", "Focus - spend Soul to heal a Mask"},
    ...
    public void show() {
        ...
        controlsTab.setPosition(centerX - 230, 860);
        abilitiesTab.setPosition(centerX + 10, 860);

        tabIndicator = new Image(Assets.getWhitePixel());
        tabIndicator.setColor(0.85f, 0.7f, 0.25f, 1f);
        tabIndicator.setSize(220, 4);

        GuideMenuController.modifyTabs(controlsTab, abilitiesTab, this);
    ...
    public void showControls() {
        ...
        for (String[] row : CONTROLS) {
            ...
            key.setPosition(x - 70f, y);

            Label desc = new Label(row[1], descStyle);
            desc.setPosition(x + 250, y);
